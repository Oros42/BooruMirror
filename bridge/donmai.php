...
if(isset($_GET['update']) && !isset($_POST['init_booru'])){
	...
	if(isset($config[$booru_dir]) && $config[$booru_dir]['enable']) {
		...
		if($dom->load($site_rss)){
			...
			for($i=$start; $i>2;$i--){
				$item=array();
				$item['page_url']=$flux_rss->item($i)->getElementsByTagName('a')->item(0)->getAttribute('href');
				...
				if(@$dom2->loadHTMLFile($item['page_url'], LIBXML_ERR_NONE)){ // get page
					if($dom2->getElementById('image')){ // if not deleted
						...
						if(!empty($data)){
							...
							file_put_contents(__DIR__.'/../mirror/'.$booru_dir.'/img/'.$item['img_name'], file_get_contents($item['img_url']));
						}
						// get stats
						...
						if(!empty($data)){
							...
							$item['tags']='';
							$data = $dom2->getElementById('tag-list')->childNodes->item(5)->getElementsByTagName('li');
							if(!empty($data)){
								foreach ($data as $tag) {
